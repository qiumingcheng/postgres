/*-------------------------------------------------------------------------
 * modules/limit.c — Limit operator (cost + build)
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
...
static void
pg_cost_limit_fn(PgPlannerCascadesContext *ctx, PgMemoGroup *group,
                 ...
                 Cost *out_startup, Cost *out_total)
{
    ...
    if (ctx->upper->limit_tuples > 0 &&
        ctx->upper->limit_tuples < input_rows)
...
void
pg_module_limit_init(void)
{
    pg_registry_register_operator(&(PgOperatorVtable){
        .op = PG_CASCADES_PHYSICAL_LIMIT,
        .name = "Limit",
        .cost_fn = pg_cost_limit_fn,
        .build_plan_fn = pg_build_limit_fn,
    });
}
