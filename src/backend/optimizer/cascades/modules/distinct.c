/*-------------------------------------------------------------------------
 * modules/distinct.c — Distinct/Unique operator (cost + build)
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
#include "miscadmin.h"
...
static void
pg_cost_unique_fn(PgPlannerCascadesContext *ctx, PgMemoGroup *group,
...
void
pg_module_distinct_init(void)
{
    pg_registry_register_operator(&(PgOperatorVtable){
        .op = PG_CASCADES_PHYSICAL_UNIQUE,
        .name = "Unique",
        .cost_fn = pg_cost_unique_fn,
        .build_plan_fn = pg_build_unique_fn,
    });
}
