...
static Plan *
pg_build_unique_fn(PgPlannerCascadesContext *ctx, PgMemoGroup *group,
...
List *pg_rule_distinct_to_unique(PgPlannerCascadesContext *ctx, PgGroupExpr *expr){PgGroupExpr *r=pg_memo_new_group_expr(ctx,PG_CASCADES_PHYSICAL_UNIQUE);r->mode=PG_PHYS_EXPR_COMPOSABLE_OP;r->inputs=expr->inputs;r->op_private=ctx->upper;return list_make1(r);}
