/*-------------------------------------------------------------------------
 * modules/project.c — Project operator (cost + build)
 *-------------------------------------------------------------------------
 */
#include "postgres.h"
                    ...
                    PgRequiredProperty *required, PgOutputProperty *output)
{
    ...
    *output = best->output;
    ...
    /* Without agg/group/window: return child directly */
    if (ctx->upper->numGroupCols == 0 &&
        !ctx->upper->hasAggs)
        return child;

    /* With agg/group: wrap with Result for targetlist */
    return (Plan *) make_result(ctx->root,
                ctx->upper->sub_tlist, NULL, child);
}
