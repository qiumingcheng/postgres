...
SELECT cov_test(101, 'C101: inner join eq', $$SELECT t1.val, t2.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id$$);
SELECT cov_test(102, 'C102: left join', $$SELECT t1.id, t2.val FROM cascades_test_j1 t1 LEFT JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id$$);
SELECT cov_test(103, 'C103: right join', $$SELECT t1.id, t2.val FROM cascades_test_j1 t1 RIGHT JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id$$);
SELECT cov_test(104, 'C104: join+order', $$SELECT t1.val, t2.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id ORDER BY t1.id, t2.id$$);
SELECT cov_test(105, 'C105: 3-table join', $$SELECT t1.val, t2.val, t3.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id JOIN cascades_test_j3 t3 ON t2.id = t3.j2_id$$);
SELECT cov_test(106, 'C106: join single-row', $$SELECT t.val, s.val FROM cascades_test_j1 t JOIN cascades_single s ON t.id = s.id$$);

\echo '=== Part 2: TRANS rules ==='
SELECT cov_test(201, 'C201: join commutativity', $$SELECT t1.val, t2.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id$$);
SELECT cov_test(202, 'C202: join associativity', $$SELECT t1.val, t2.val, t3.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id JOIN cascades_test_j3 t3 ON t2.id = t3.j2_id$$);
SELECT cov_test(203, 'C203: join left asscom', $$SELECT t1.val, t2.val, t3.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.val = t2.val JOIN cascades_test_j3 t3 ON t2.val = t3.val$$);
SELECT cov_test(204, 'C204: outer join elim', $$SELECT t1.id, t2.val FROM cascades_test_j1 t1 LEFT JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id WHERE t2.val IS NOT NULL$$);
SELECT cov_test(205, 'C205: inner to semi', $$SELECT t1.* FROM cascades_test_j1 t1 JOIN cascades_unique t2 ON t1.id = t2.id$$);
SELECT cov_test(206, 'C206: predicate pushdown join', $$SELECT t1.val, t2.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id WHERE t1.val > 5 AND t2.val > 3$$);
SELECT cov_test(207, 'C207: limit pushdown join', $$SELECT t1.val, t2.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id LIMIT 10$$);
SELECT cov_test(208, 'C208: merge filter join', $$SELECT t1.val, t2.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id WHERE t1.val BETWEEN 10 AND 30$$);
SELECT cov_test(209, 'C209: prune empty join', $$SELECT t.val, e.val FROM cascades_test_j1 t JOIN cascades_empty e ON t.id = e.id$$);
SELECT cov_test(210, 'C210: merge join child proj', $$SELECT a, c FROM (SELECT id AS a, val FROM cascades_test_j1) s1 JOIN (SELECT j1_id AS a, val AS c FROM cascades_test_j2) s2 ON s1.a = s2.a$$);
...
SELECT cov_test(501, 'C501: self join', $$SELECT t1.id, t2.id FROM cascades_test_t t1 JOIN cascades_test_t t2 ON t1.a = t2.b$$);
SELECT cov_test(502, 'C502: 4-table join', $$SELECT t1.id, t2.val, t3.val, t4.val FROM cascades_test_j1 t1 JOIN cascades_test_j2 t2 ON t1.id = t2.j1_id JOIN cascades_test_j3 t3 ON t2.id = t3.j2_id JOIN cascades_test_t t4 ON t3.val = t4.b$$);
...
SELECT cov_test(702, 'C702: distinct agg combo', $$SELECT count(DISTINCT a), count(DISTINCT b) FROM cascades_test_t$$);
